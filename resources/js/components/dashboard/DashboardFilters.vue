...
const hasActiveFilters = () => {
    return (
        dashboardFilters.value.search ||
        dashboardFilters.value.template_id ||
        dashboardFilters.value.status ||
        dashboardFilters.value.executor_id
    );
};
...
<template>
    <div class="mb-6 flex flex-col items-center gap-3 sm:flex-row">
        ...
        <Select
            v-model="dashboardFilters.template_id"
        ...
        >
            ...
            <SelectContent>
                <SelectItem value="App\States\ProcessInstance\Running"
                    >Đang chạy</SelectItem
                >
                <SelectItem value="App\States\ProcessInstance\Completed"
                    >Đã hoàn thành</SelectItem
                >
                <SelectItem value="App\States\ProcessInstance\Cancelled"
                    >Đã hủy</SelectItem
                >
            </SelectContent>
        </Select>
        ...
        <Select
            v-model="dashboardFilters.executor_id"
            @update:modelValue="applyFilters"
        >
            <SelectTrigger class="w-full sm:w-[160px]">
            ...
            <SelectContent>
                ...
                <SelectItem
                    v-for="user in options.executors"
                    :key="user.id"
