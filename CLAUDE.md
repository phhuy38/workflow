...
- **MANDATORY**: All commands (except `git` commands) MUST be executed inside the `laradock` environment located at `/home/huyph/repositories/laradock`.
- You must execute these commands using the `laradock` user and change to the `workflow` directory inside the workspace before running commands. For example: `cd /home/huyph/repositories/laradock && docker-compose exec --user=laradock workspace bash -c "cd workflow && your command here"`.
